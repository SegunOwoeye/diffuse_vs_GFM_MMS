...
)
{
    ...
    // [6.1] Enforce positivity and check reconstructed interface states
    for (int i = 0; i < L - 1; ++i) {
    ...
    // [6.3] Conservative update on interior cells of the line
    ...
    // [6.4] Simple boundary copy on the line
    if (L >= 2) {
...
)
{
    ...
    // [9.2] Geometry (disabled level set → flat normals)
    std::vector<double> phi_geom = phi;
    ...
    // [9.4] No level set update for Sod test
