"""Legacy graphing-side error norm helpers.

The C++ quantitative runner now owns report-facing error metrics. This module
keeps the older graphing scripts working by preserving their CSV folder
convention and exact-vs-finest-reference behaviour.
"""
...
def load_csv(path):
    """Load graphing CSVs into the field names expected by legacy tables."""
...
def compute_errors_exact(files):
    """Compare numerical 1D outputs against an exact/reference CSV if present."""
...
def compute_errors_reference(files):
    """Use the finest available numerical grid as the reference solution."""
