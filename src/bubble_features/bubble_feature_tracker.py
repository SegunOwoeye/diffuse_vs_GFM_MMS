#!/usr/bin/env python3
"""Helium shock-bubble feature tracker used by the quantitative suite.

The tracker deliberately separates material-interface features from wave
features:

* upstream/downstream helium interfaces are taken from the material interface
  (`phi0=0` for GFM/SIM, `alpha1=0.5` for DIM/Allaire),
* the jet head is found from centreline interface-crossing clusters,
* the transmitted shock is found from pressure/density-gradient candidates and
  selected with temporal branch tracking.

All physical measurements come from the solver CSV grid. Schlieren PNGs are
used only to generate visual overlays for validation/debugging.
"""
