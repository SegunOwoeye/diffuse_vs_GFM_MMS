...
cores_3d="${CORES_3D:-${CORES:-${OMP_NUM_THREADS:-6}}}"
...
for t in "${tests_1d[@]}"; do
    ...
    OMP_NUM_THREADS=$cores_1d OMP_SCHEDULE=dynamic ./sm_main_1d configs/toro/$t.txt
done
...
for t in "${tests_2d[@]}"; do
    echo "[2D] Running $t"
    OMP_NUM_THREADS=$cores_2d OMP_SCHEDULE=dynamic ./sm_main_2d configs/toro/$t.txt
done
...
for t in "${tests_3d[@]}"; do
    echo "[3D] Running $t"
    OMP_NUM_THREADS=$cores_3d OMP_SCHEDULE=dynamic ./sm_main_3d configs/toro/$t.txt
done
